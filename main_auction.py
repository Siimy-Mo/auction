    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(
        ...
        for e in range(params.epoch):
            ...
            for b in range(total_batch):
                ...
                u_train,bpp_train,sl_train,bpf_train,cpp_train,cni_train \
                    = data.Get_next_batch(Dataset, train_dataset, train_startpos, params.batch_size)
                _, train_loss, train_pos_loss, train_neg_loss = psammodel.step(session, u_train,bpp_train,sl_train,bpf_train,cpp_train,cni_train )
            ...
            u_test,bpp_test,sl_test,bpf_test,cpp_test,cni_test  = data.Get_next_batch(Dataset, test_dataset, 0, len(test_dataset))
            
            #print("Start Performance Test of point model!\n")
            bs, itememb, allproductemb = psammodel.step(session, u_test,bpp_test,sl_test,bpf_test,cpp_test,cni_test ,True)
            all_hr = 0
            all_mrr = 0
            all_ndcg = 0
        ...
        with open(filename, 'a+') as f:
            ...
            f.write(bestinfo+'\n')
    x_data = range(100)
    l1=plt.plot(x_data,HR_list,'r--',label='HR_list')
    l2=plt.plot(x_data,MRR_list,'g--',label='MRR_list')
    l3=plt.plot(x_data,NDCG_list,'b--',label='NDCG_list')
    plt.plot(x_data,HR_list,'ro-',x_data,MRR_list,'g+-',x_data,NDCG_list,'b^-')
...
def main(args):
    ...
    # psammodel = PSAM.PSAM(Embed, args)model_test
